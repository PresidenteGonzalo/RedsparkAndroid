...
public class BitmapCache {
    ...
    private BitmapCache() {
        if (this.cacheSize == 0) {
            Log.d("BitmapCache", "Yikes, BitmapCache.cacheSize not set!");
            return;
        }
        if (maxW == 0 || maxH == 0) {
            Log.d("BitmapCache", "Yikes, set maxW and maxH to get bitmap scaling.");
        }
    ...
    public static Bitmap makePlaceholderImage() {
        int h = thumbH;
        int w = thumbW;
        Bitmap bm = Bitmap.createBitmap(w, h, Bitmap.Config.RGB_565);
        Canvas canvas = new Canvas(bm);
        Paint darkPaint = new Paint();
        darkPaint.setColor((0xFF << 24) + new Random().nextInt(0xFFFFFF + 1));
        canvas.drawRect(0, 0, w, h, darkPaint);
        return bm;
    }
    ...
    public synchronized void setBitmap(String key, Bitmap bitmap) {
        if (mMemoryCache.get(key) == null) {
            ...
            if (bitmap == null) {
                Log.d("BitmapCache", "Yikes, bitmap too big " + key);
    ...
    public synchronized Bitmap getBitmap(String key) {
        ...
        /*if (bm == null) {
            Log.d("BitmapCache", "BMCache miss " + key);
        } else {
            Log.d("BitmapCache", "BMCache hit " + key);
        }*/
        return bm;
    }
}
